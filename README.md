...
**No external requests.** The fonts — [Lalezar](https://fonts.google.com/specimen/Lalezar) for display and [Vazirmatn](https://fonts.google.com/specimen/Vazirmatn) for body text — are self-hosted as woff2, split into `arabic` / `latin` / `latin-ext` subsets with `unicode-range` so a browser fetches only what the page actually needs. Nothing is loaded from a CDN, and nothing is phoned home.
...
- **Digits.** Numbers display as Persian digits, but the parser accepts Persian, Arabic-Indic, and ASCII digits, plus any of `-`, `−`, `–`, `—` as a minus.
